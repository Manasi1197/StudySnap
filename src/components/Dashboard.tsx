...
const Dashboard: React.FC<DashboardProps> = ({ currentPage = 'dashboard', onNavigate }) => {
  ...
  const getAvatarUrl = () => {
    ...
    return `https://ui-avatars.com/api/?name=${encodeURIComponent(getDisplayName())}&size=128&background=6366f1&color=ffffff`;
  };
  ...
  const handleNavigateFromSubPage = (targetView: 'audio' | 'flashcards' | 'take-quiz') => {
    ...
    switch (targetView) {
      case 'audio':
        ...
        setSubPageData({
          quiz: quiz,
          audioUrl: audioUrl || 'https://www.soundjay.com/misc/bell-ringing-05.wav',
          title: quiz.title,
                ...
                return (
                  <div key={index} className="bg-white rounded-xl p-4 sm:p-6 border border-gray-200">
                    <div className="flex items-center justify-between mb-4">
                      <div className={`w-10 sm:w-12 h-10 sm:h-12 ${card.color} rounded-xl flex items-center justify-center`}>
                        <Icon className="w-5 sm:w-6 h-5 sm:h-6 text-white" />
                      </div>
                    </div>
                    <div className="text-2xl sm:text-3xl font-bold text-gray-900 mb-1">{card.number}</div>
            ...
            <div className="bg-white rounded-xl border border-gray-200">
              ...
              <div className="p-4 sm:p-6">
                <div className="space-y-4">
                  ...
                  <div className="hidden sm:grid sm:grid-cols-4 gap-4 text-sm font-medium text-gray-500 pb-2">
                    <div>Course Name</div>
                    <div>Progress</div>
                    <div>Status</div>
                    <div></div>
                  ...
                  {continuelearning.map((course, index) => (
                    <div key={index} className="sm:grid sm:grid-cols-4 gap-4 items-center py-4 border-t border-gray-100 space-y-3 sm:space-y-0">
                      <div className="flex items-center space-x-3">
                        <div className={`w-10 h-10 ${course.color} rounded-lg flex items-center justify-center text-white text-lg flex-shrink-0`}>
                          {course.icon}
                        </div>
                      ...
                      <div className="flex items-center space-x-3">
                        ...
                        <span className="text-sm font-medium text-gray-600 whitespace-nowrap">{course.progress}%</span>
                      </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                  {recommendations.map((rec, index) => (
                    <div key={index} className="group cursor-pointer">
                      <div className={`${rec.color} rounded-xl p-6 sm:p-8 mb-4 group-hover:scale-105 transition-transform duration-300`}>
                        <div className="text-3xl sm:text-4xl mb-4">{rec.icon}</div>
                      </div>
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      ...
      {!isInSubPage && (
        <div className={`fixed lg:static inset-y-0 left-0 z-50 w-64 bg-white border-r border-gray-200 flex flex-col transform transition-transform duration-300 ease-in-out lg:transform-none ${
      ...
      <div className="flex-1 flex flex-col min-w-0">
        ...
        {/* Dashboard Content */}
        <main className={`flex-1 ${!isInSubPage && currentPage !== 'quiz-generator' && currentPage !== 'study-rooms' && currentPage !== 'materials' && currentPage !== 'achievements' && currentPage !== 'settings' && currentPage !== 'help' && currentPage !== 'marketplace' ? 'p-4 sm:p-8' : ''}`}>
          {/* Conditional container width based on current page */}
          {currentPage === 'quiz-generator' || isInSubPage || currentPage === 'study-rooms' || currentPage === 'materials' || currentPage === 'achievements' || currentPage === 'settings' || currentPage === 'help' || currentPage === 'marketplace' ? (
            // Full width for Quiz Generator, sub-pages, Study Rooms, Materials, Achievements, Settings, Help, and Marketplace
