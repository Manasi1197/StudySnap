...
export async function translateText(
  ...
  if (!LINGO_API_KEY || LINGO_API_KEY === 'your_lingo_api_key' || LINGO_API_KEY === 'api_przi40vhvtidf7uri0klbece') {
    console.warn('⚠️ Lingo API key is not configured or using placeholder, using mock translation');
  ...
  try {
    ...
    // Always use mock translation as fallback for any error - don't throw
...
export async function testLingoConnection(): Promise<boolean> {
  try {
    ...
    if (!LINGO_API_KEY || LINGO_API_KEY === 'your_lingo_api_key' || LINGO_API_KEY === 'api_przi40vhvtidf7uri0klbece') {
      console.warn('⚠️ Lingo API key is not configured or using placeholder');
      return false;
    }
